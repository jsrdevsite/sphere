...
<?php
			...
			&& in_array($extension, $allowedExts)) {
				    ...
				    echo "Return Code: " . $_FILES["file"]["error"][$key] . "<br>";
			  			...
			  			$file = move_uploaded_file($_FILES["file"]["tmp_name"][$key],
				 		  ...
				 		  mysql_query("INSERT INTO wp_bg_image (id, image, file_name) values ('','".$path."','".$_FILES["file"]["name"][$key]."')");
						}
				}
	...
	$result = $wpdb->get_results("select DISTINCT file_name,id, image from wp_bg_image");
	$row = count($result);
	$num_col = ceil($row/4);
	...
	if($row) {
		...
		echo "</table><input type='hidden' name='bg_option' value='yes'></form>";
	}
